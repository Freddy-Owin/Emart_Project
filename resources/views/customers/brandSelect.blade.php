    ...
    <div class="row w-100">
        <div class="col-3 position-fixed bg-dark border-end border-info" style="height: 1000px;">
            <h3 class="mt-5 text-center">
                <a href="{{ route('brands-view') }}" class="text-success mt-5 text-decoration-none text-center">Brands</a>
            </h3>
            @foreach ($brands as $tag)
            <ul class="list-group list-group-flush text-center">
                <a href="{{ route('brand-select', $tag->id) }}" class="list-group-item list-group-item-action bg-dark text-decoration-none text-success">{{ $tag->name }}</a>
            </ul>
            @endforeach
        </div>
